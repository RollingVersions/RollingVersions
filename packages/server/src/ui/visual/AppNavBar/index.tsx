...
export default function AppNavBar({children}: NavBarProps) {
  return (
    <nav className="flex items-center text-2xl px-10 bg-white h-16 sticky top-0 z-50">
      <Link to="/">
...
export function AppNavBarLink({children, to}: AppNavBarLinkProps) {
  if (!to) {
    return <span className="font-popins">{children}</span>;
  }
  return (
    <Link to={to} className="font-popins">
      {children}
    </Link>
  );
}
